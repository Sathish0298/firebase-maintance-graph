...
<head>
    ...
    <style>
        body { padding: 20px; font-family: Arial, sans-serif; background: #eef2f7; }
        .container { max-width: 800px; margin: auto; background: #ffffff; padding: 20px; border-radius: 10px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); }
        ...
        function fetchData() {
            fetch('/update')
                ...
                .then(data => {
                    ...
                    if (responseDiv.innerHTML !== data.content) {
                        responseDiv.innerHTML = data.content;  // ✅ Update only if new data exists
                    }
                });
        }
        setInterval(fetchData, 30000);  // ✅ Refresh every 30 seconds only if new data exists
    </script>
</head>
<body>
    <!-- Navbar -->
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ms-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" id="graph-link" target="_blank">Graph View 📊</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <script>
        // ✅ Dynamic Graph URL (removes static refresh_time)
        document.getElementById("graph-link").href = `https://firebase-maintance-graph0298.streamlit.app/?t=${Date.now()}`;
    </script>
